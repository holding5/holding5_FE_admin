...
import { hopeMessageMock } from "../utils/HopeMessageMock"; // mock 데이터
...
const columns = [
  ...
  { key: "log", label: "비고", width: "100px",
    filterable: true,
    options: ["전체", "희망", "희망>생명", "삭제"]
   },
];

const sortableKeys = [ "date", "reports", "coments", "lifeSaves", "courage" ];

const HopeMessageTable = ({ itemsPerPage = 10 }) => {
  ...
  // ✅ 필터 적용
  const filteredData = hopeMessageMock.filter((row) => {
